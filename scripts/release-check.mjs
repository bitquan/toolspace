...
class ReleaseValidator {
    ...
    try {
      ...
      if (!proPlan) {
        ...
        if (proPlan.stripePriceId === 'price_XXXXXXXXXXXXXXXX') {
          this.errors.push('Pro plan stripePriceId contains placeholder "price_XXXXXXXXXXXXXXXX"');
        } else if (!proPlan.stripePriceId || !proPlan.stripePriceId.startsWith('price_')) {
          this.errors.push(`Pro plan stripePriceId invalid: ${proPlan.stripePriceId}`);
        } else {
          this.passed.push(`Pro plan stripePriceId: ${proPlan.stripePriceId}`);
        }

        if (proPlan.price?.amount !== 900) {
          this.errors.push(`Pro plan amount should be 900 cents ($9), got ${proPlan.price?.amount}`);
        } else {
      ...
      if (!proPlusPlan) {
        ...
        if (proPlusPlan.stripePriceId === 'price_YYYYYYYYYYYYYYYY') {
          this.errors.push('Pro+ plan stripePriceId contains placeholder "price_YYYYYYYYYYYYYYYY"');
        } else if (!proPlusPlan.stripePriceId || !proPlusPlan.stripePriceId.startsWith('price_')) {
          this.errors.push(`Pro+ plan stripePriceId invalid: ${proPlusPlan.stripePriceId}`);
        } else {
          this.passed.push(`Pro+ plan stripePriceId: ${proPlusPlan.stripePriceId}`);
        }

        if (proPlusPlan.price?.amount !== 1900) {
          this.errors.push(`Pro+ plan amount should be 1900 cents ($19), got ${proPlusPlan.price?.amount}`);
        } else {
  ...
  async validateGitHubSecrets() {
    ...
    if (!process.env.GITHUB_ACTIONS) {
      this.warnings.push('Not running in GitHub Actions - skipping secrets validation');
      return;
    }
  ...
  getSecretType(secretName, value) {
    if (secretName.includes('STRIPE_SECRET_KEY')) {
      return value.startsWith('sk_live_') ? 'live' : 
             value.startsWith('sk_test_') ? 'test' : 'unknown';
    }
  ...
  validateWorkflowFiles() {
    ...
    try {
      ...
      const prodWorkflowPath = join(__dirname, '..', '.github', 'workflows', 'prod-release.yml');
      const prodContent = readFileSync(prodWorkflowPath, 'utf8');

      if (prodContent.includes('your-prod-project-id')) {
        this.errors.push('prod-release.yml contains placeholder "your-prod-project-id"');
      } else {
        this.passed.push('prod-release.yml placeholders updated');
      }

      // Check staging-release.yml
      const stagingWorkflowPath = join(__dirname, '..', '.github', 'workflows', 'staging-release.yml');
      const stagingContent = readFileSync(stagingWorkflowPath, 'utf8');

      if (stagingContent.includes('your-prod-project-id')) {
        this.errors.push('staging-release.yml contains placeholder "your-prod-project-id"');
      } else {
      ...
      if (!stagingContent.includes('permissions:')) {
        this.errors.push('staging-release.yml missing explicit permissions block');
      } else {
