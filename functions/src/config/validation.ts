...
export function validateProductionConfig(): void {
  ...
  try {
    ...
    if (proPlan && proPlan.stripePriceId) {
      if (proPlan.stripePriceId === "price_XXXXXXXXXXXXXXXX") {
        errors.push("Pro plan stripePriceId contains placeholder 'price_XXXXXXXXXXXXXXXX'");
      } else if (!proPlan.stripePriceId.startsWith("price_")) {
        errors.push(`Pro plan stripePriceId has invalid format: ${proPlan.stripePriceId}`);
      }
    } else {
    ...
    if (proPlusPlan && proPlusPlan.stripePriceId) {
      if (proPlusPlan.stripePriceId === "price_YYYYYYYYYYYYYYYY") {
        errors.push("Pro+ plan stripePriceId contains placeholder 'price_YYYYYYYYYYYYYYYY'");
      } else if (!proPlusPlan.stripePriceId.startsWith("price_")) {
        errors.push(`Pro+ plan stripePriceId has invalid format: ${proPlusPlan.stripePriceId}`);
      }
    } else {
      errors.push("Pro+ plan stripePriceId is not configured");
    }
    ...
    if (proPlan && proPlan.price.amount !== 900) {
      errors.push(`Pro plan amount should be 900 cents ($9), got ${proPlan.price.amount}`);
    }

    if (proPlusPlan && proPlusPlan.price.amount !== 1900) {
      errors.push(`Pro+ plan amount should be 1900 cents ($19), got ${proPlusPlan.price.amount}`);
    }

  } catch (error: any) {
    errors.push(`Failed to load pricing configuration: ${error?.message || String(error)}`);
  }
  ...
  if (!projectId) {
    ...
    errors.push("Firebase project ID contains placeholder 'your-prod-project-id'");
  }
  ...
  if (errors.length > 0) {
    ...
    if (process.env.NODE_ENV === "production") {
      console.error("💥 BLOCKING PRODUCTION STARTUP DUE TO CONFIGURATION ERRORS");
...
export function getConfigSummary(): object {
  ...
  return {
    ...
    stripeKeyType: stripeKey ? (stripeKey.startsWith("sk_live_") ? "live" : 
                                stripeKey.startsWith("sk_test_") ? "test" : "unknown") : "none",
    webhookSecretPresent: !!(process.env.STRIPE_WEBHOOK_SECRET || functions.config().stripe?.webhook_secret),
